...
function newAddress(recipientId){
    ...
    bot.setListener(recipientId, 'location', setLocationCheck);
...
function writeAddress(recipientId){
    ...
    if(bot.getListener(recipientId, 'location')){
        bot.deleteListener(recipientId, 'location');
    }

    bot.setListener(recipientId, 'address', writeAddressCheck);
    bot.callSendAPI(messageData);
}
...
function confirmAddress(recipientId){
    ...
    bot.setListener(recipientId, 'address-name', writeAddressComplete);
    bot.callSendAPI(messageData);
}
...
function searchProducts(recipientId, query, index){
    ...
    if(index == 0){
        renderSearchInitialMessage(recipientId);
        bot.sendTypingOn(recipientId);
    }
...
function renderSearchInitialMessage(recipientId){
    var messageData = {
        recipient: {
            id: recipientId
        },
