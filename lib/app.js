...
import {Customer, User, Consumer, ConsumerAddress, Category, Product, Order, OrderItem, OrderState, Cart, PaymentMethod, CreditCard} from './models/ParseModels';
...
function signUp(recipientId, facebookId, conversationToken){
    return createUser(recipientId, facebookId, conversationToken).then(()=>{
        let user = getData(recipientId, 'user');



        return createConsumer(recipientId, user.rawParseObject, conversationToken).then(consumer => {
...
function createUser(recipientId, facebookId, conversationToken){
    let user = new User({facebookId: parseInt(facebookId)});
    return bot.getFacebookUserData(facebookId, conversationToken).then(data => {
        delete data.id;
...
function createConsumer(recipientId, user, conversationToken){
...
function renderAddProductConfirmation(recipientId, productId){
    return new Parse.Query(Product).get(productId).then(
        product => {
            let messageData = {
                recipient: {
                    id: recipientId
...
function renderCartDetails(recipientId){
    ...
    items.forEach(function(value, key) {
        if (elements.length <= bot.limit) {
            new Parse.Query(Product).get(key).then(
            product => {
...
function renderCreditCard(recipientId){
    ...
    if(!_.isEmpty(consumer.user)) {
        CreditCard.loadInStore(store, recipientId, consumer.user).then(() => {
