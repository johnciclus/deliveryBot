...
bot.rules.set('signup', sendSignUp)
...
function signUp(facebookId, conversationId, conversationToken){
    console.log(facebookId);
    console.log(conversationId);
    console.log(conversationToken);

    createUser(facebookId, conversationId, conversationToken)
}
...
function createUserBK(recipientId){
    let user = new ParseModels.User({recipientId});
    return bot.getFacebookUser(recipientId).then(data => {
        return user.signUpWithFacebook(data).then(() => {
            return user.saveInStore(store, recipientId).fail(error => {
                console.log('Error code: ' + error.message);
            });
...
function createConsumer(user, recipientId){
...
function getUser(recipientId){
    ...
    if(typeof userObj == 'undefined'){
        return loadUser(recipientId).then(()=>{
            let _userObj = getData(recipientId, 'user');

            if(typeof _userObj == 'undefined'){
                return createUserBK(recipientId).then( () => {
...
function renderSignUp(recipientId) {
    ...
    let image_url = SERVER_URL+"assets/love.jpg";
    let messageData = {
        recipient: {
            id: recipientId
        },
...
bot.app.post('/registerUser', function (req, res) {
    ...
    signUp(data.uid, data.psid, data.accessToken);
