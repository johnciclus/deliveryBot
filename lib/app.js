...
function sendProducts(recipientId, category, proIdx){
    ...
    Parse.Cloud.run('getProducts', { businessId: BUSINESS_ID, category: category }).then(result => {
        if(result.hasOwnProperty('categories')){
            ...
            renderProducts(recipientId, result.products, proIdx);
        }
    },
    function(error) {
        console.log('error');
...
function renderProducts(recipientId, products, index){
    ...
    if( idx > (index+1)*bot.limit ){
        buttons.push({
            ...
            payload: "SendProducts-"+category+"-"+(index+1),
        });
        ...
        Parse.Promise.when(promises).then(function(modifiers) {
            ...
            if(modifiers.length != item.modifiers.length){
                ...
                modifiers.forEach(function (modifier) {
                    ...
                    Parse.Promise.when(promises).then(function (elements) {
