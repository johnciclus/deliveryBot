...
_firebase2.default.database().ref('/states/dev').once('value').then(function (snapshot) {
...
var reducer = function reducer() {
    ...
    switch (action.type) {
        ...
        case types.SET_USER:
            var user = (0, _ParseUtils.extractParseAttributes)(data.user);
            (0, _objectAssign2.default)(state.userData[data.recipientId], { user: user });
            return _extends({}, state);
...
function sendMenu(recipientId) {
    ...
    user.registered(function (user_registered) {
        if (typeof user_registered == 'undefined') {
            bot.getFacebookUser(recipientId, function (data) {
                user.signUp(Object.assign(data, { username: recipientId.toString(), password: recipientId.toString(), facebookId: recipientId }), {
                    success: function success(_user) {

                        console.log(store.dispatch(Actions.setUser(recipientId, _user)));
                        console.log('Set user');

                        consumer = new ParseModels.Consumer();
                        consumer.setUser(_user);
                        consumer.save(undefined, {
                            success: function success(_consumer) {
                                loadMenuData(recipientId, _user);
                            },
                ...
                //signUp(recipientId, userData, loadMenuData);
            });
        } else {
            console.log(user_registered);

            loadMenuData(recipientId, user_registered);
        }
    });
    /*
...
function loadMenuData(recipientId, user) {
...
function increaseOneProduct(recipientId, productId) {
    ...
    new Parse.Query(ParseModels.OrderItem).get(item.id, {
        ...
        error: function error(orderItem, _error7) {
            console.log('error');
            console.log(_error7);
        }
    });
...
function saveCart(recipientId) {
    ...
    Parse.Object.saveAll(items, {
        ...
        error: function error(user, _error10) {
            // Execute any logic that should take place if the save fails.
            // error is a Parse.Error with an error code and message.
            console.log('Failed to create new object, with error code: ' + _error10.message);
        }
    });
}
...
function saveOrder(recipientId) {
    ...
    order.save(undefined, {
        ...
        error: function error(user, _error11) {
            // Execute any logic that should take place if the save fails.
            // error is a Parse.Error with an error code and message.
            console.log('Failed to create new object, with error code: ' + _error11.message);
...
function clearCart(recipientId) {
    ...
    items.forEach(function (value, key) {
        new Parse.Query(ParseModels.OrderItem).get(value.id, {
            ...
            error: function error(orderItem, _error13) {
                console.log('error');
