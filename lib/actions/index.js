 ...
 */
export function loadConsumer(user) {
    if (user == null) return
    return dispatch => {
 ...
 */
export function loadConsumerAddresses(consumer) {
...
export function getConsumerAndAddresses(user){
    return (dispatch, getState) => {
        return dispatch(loadConsumer(user)).then(() => {
 ...
 * Load Consumer of given user
 */
export function loadCustomer(businessId) {
    if (businessId == null) return
    return dispatch => {
        return new Parse.Query(Customer).contains('businessId', businessId).limit(1).first().then(
            customer => {
                dispatch({type: types.CUSTOMER_LOADED, data: {customer}})
            }
        ).fail(e => {
            dispatch({type: types.CUSTOMER_NOT_FOUND, data: {businessId}})
        })
    };
}
