...
if(typeof payloadFunction=='function'){if(params.length==4){payloadFunction(senderID,recipientID,params[1],params[2],params[3]);}else if(params.length==3){payloadFunction(senderID,recipientID,params[1],params[2]);}else{payloadFunction(senderID,recipientID,params[1]);}}}else{payloadFunction=payloadRules.get(quickReplyPayload);if(payloadFunction){payloadFunction(senderID,recipientID);}}}else if(messageText){var userListeners=listener[senderID];var existRule=false;var ruleFunction=undefined;messageText=messageText.toLowerCase();rules.forEach(function(func,key){if(messageText.includes(key)){ruleFunction=func;existRule=true;}});if(existRule){ruleFunction(senderID,recipientID);}else{if(!_.isEmpty(userListeners)){if(!buffer[senderID]){buffer[senderID]={};}var keys=Object.keys(userListeners);var key=keys.shift();while(key){if(userListeners[key].type=='text'){buffer[senderID][key]=messageText;userListeners[key].callback(senderID,recipientID);existRule=true;}delete userListeners[key];key=keys.shift();}}else{defaultSearch(senderID,recipientID,messageText);}}}else if(messageAttachments){if(messageAttachments[0].type=='location'){var location=messageAttachments[0].payload.coordinates;var _userListeners=listener[senderID];//console.log(senderID);
//console.log(typeof senderID);
...
var search=payloadRules.get('Search');if(search){search(recipientId,senderId,query);}};function clearListener(recipientId){var userListener=listener[recipientId];if(typeof userListener!='undefined'){listener[recipientId]={};}}/*
