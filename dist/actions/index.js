 ...
 * Load Consumer of given user
 */function loadUser(recipientId){return function(dispatch){return new Parse.Query(_ParseModels.Consumer).equalTo('recipientId',parseInt(recipientId)).first().then(function(consumer){if(consumer){return new Parse.Query(_ParseModels.User).get(consumer.get('user').id).then(function(user){dispatch({type:types.USER_LOADED,data:{recipientId:recipientId,user:user}});}).fail(function(e){dispatch({type:types.USER_NOT_FOUND,data:{user:user}});});}}).fail(function(e){dispatch({type:types.CONSUMER_NOT_FOUND,data:{}});});/*
...
* saved successfully.
*/function updateConsumer(senderId,consumerData){return function(dispatch){dispatch({type:types.UPDATE_CONSUMER,data:consumerData});var consumer=new _ParseModels.Consumer();consumer.objectId=consumerData.objectId;consumer.save(consumerData).then(function(consumer){var username=consumer.get('email')+BUSINESS[senderId].BUSINESS_ID;consumer.get('user').save({username:username}).then(function(u){dispatch({type:types.CONSUMER_UPDATED,data:{consumer:consumer}});dispatch({type:types.HIDE_PROFILE});}).fail(function(e){dispatch({type:types.CONSUMER_UPDATE_ERROR,data:{consumer:consumer}});});}).fail(function(e){dispatch({type:types.CONSUMER_UPDATE_ERROR,data:{consumer:consumer}});});};}/**
...
* Email Login Action
*/function emailLogin(senderId,userData,mainDispatch){mainDispatch({type:types.EMAIL_LOGIN});return function(dispatch){Parse.User.logIn(userData.email+BUSINESS[senderId].BUSINESS_ID,userData.password).then(function(user){dispatch({type:types.EMAIL_LOGIN_SUCCESS,data:user});mainDispatch(push('/'));dispatch(loadConsumer(user,mainDispatch));}).fail(function(e){dispatch({type:types.EMAIL_LOGIN_ERROR,data:e});});};}/**
* Email Login Action
*/function emailRegister(senderId,userData,mainDispatch){mainDispatch({type:types.EMAIL_REGISTER});return function(dispatch){Parse.User.signUp(userData.email+BUSINESS[senderId].BUSINESS_ID,userData.password).then(function(user){dispatch({type:types.EMAIL_REGISTER_SUCCESS,data:{user:user,userData:userData}});delete userData.password;delete userData.passwordConfirmation;userData.user=user;dispatch(createConsumer(userData,mainDispatch));}).fail(function(e){dispatch({type:types.EMAIL_REGISTER_ERROR,data:e});});};}/**
...
*/function loadPointSales(senderId){var params={businessId:BUSINESS[senderId].BUSINESS_ID};return function(dispatch){Parse.Cloud.run('getPointSales',params).then(function(results){dispatch({type:types.POINT_OF_SALES_LOADED,data:results});});};}/**
* Geolocation error.
