...
_node2.default.initialize(PARSE_APP_ID, '', PARSE_CLIENT_KEY);
...
function sendMenu(recipientId) {
    ...
    store.dispatch(Actions.loadCustomer(BUSINESS_ID)).then(function () {
        new _node2.default.Query(ParseModels.User).equalTo('facebookId', recipientId).first().then(function (user) {
            if (user) {
                ...
                //console.log(bot.getFacebookUser(recipientId));
            }
...
function renderProductsInitialMessage(recipientId, categoryId) {
    return new _node2.default.Query(ParseModels.Category).get(categoryId).then(function (category) {
        var messageData = {
            recipient: {
                id: recipientId
            },
...
function renderAddProductConfirmation(recipientId, productId) {
    return new _node2.default.Query(ParseModels.Product).get(productId).then(function (product) {
        var messageData = {
            recipient: {
                id: recipientId
            },
